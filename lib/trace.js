...
var breadthFirst = require('algorithms/graph').breadthFirstSearch;
...
function Trace(loader, traceCache) {
  ...
  // Depependency graph model for ordered traversals
    ...
    .then(function(load) {
      ...
      .then(function() {
        return Promise.all(load.deps.map(function(dep) {
          // Save edges in the graph for ordered traversals
          self.graph.addEdge(load.name, load.depMap[dep]);
...
Trace.prototype.getOrderedLoads = function (name) {
  var self = this;

  return this.getAllLoadRecords(name)
    .then(function (loads) {
      var preOrder = [];
      var postOrder = [];

      breadthFirst(self.graph, name, {
        leaveVertex: function (vertex) {
          preOrder.push(vertex);
        }
      });

      depthFirst(self.graph, name, {
        leaveVertex: function (vertex) {
